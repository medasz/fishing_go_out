...
- **多情报源聚合**（5 个实时 API 源 + 1 个本地引擎，免 Key 或免费 Key）：
  - **实时 API 类**：URLhaus (abuse.ch，**需免费 Auth-Key**)、AlienVault OTX、**VirusTotal（多引擎扫描，需免费 API Key）**、**微步 ThreatBook（国内威胁情报，需免费 API Key）**、**Pulsedive（综合威胁情报，需免费 API Key）**
...
- **持久化白名单（免检测）**：常用安全站点加入白名单后**直接放行、跳过检测**；预置 41 个常用可信站点，支持搜索过滤、按当前页一键添加、恢复预置、清空，数据持久化保存（重启不丢）
- **手动放行**：用户确认继续访问后域名记入会话级别白名单，当前会话内不再二次拦截
- **配置导入 / 导出**：一键导出白名单 / API Key / 情报源开关为 JSON 备份，或导入迁移到其它浏览器
...
### 白名单管理（免检测）
展开弹窗中的 **⭐ 我的白名单（免检测）**，管理常用安全站点：

- **搜索框**：输入子串实时过滤（如 `git` 同时命中 `github.com`、`gitee.com`），忽略大小写
- **添加**：手动输入域名（自动去协议/路径/端口，转小写）后回车，或点 **+ 添加当前页面域名** 一键加入正在浏览的站点；也支持直接填 **IP** 或 **IP 段**：
  - 精确 IP：`192.168.1.1`
  - CIDR 段：`192.168.1.0/24`（覆盖该网段全部地址）
  - 通配段：`192.168.1.*` 或 `10.*.*.*`
- **移除**：每条右侧 `✕` 删除单个域名
- **恢复预置**：误删后一键补齐 41 个预置站点（不会删掉你的自定义项）
- **清空白名单**：清空全部自定义与预置项

匹配规则：
- **域名**：精确匹配 `github.com`，并自动覆盖子域名 `gist.github.com`、`www.github.com`；不会误命中 `evilgithub.com` 或 `github.com.evil.com`
- **IP / IP 段**：被访问地址为 IPv4 时，若与白名单中的精确 IP 相等、落在 CIDR 段内（如 `192.168.1.0/24`）、或逐段匹配通配（如 `192.168.1.*`、 `10.*.*.*`）即放行。常见用法：把路由器 / 内网设备网段 `192.168.1.0/24` 加入，即可免拦截正常访问内网

命中白名单的域名 / IP 在导航前直接放行、跳过检测。

> 预置白名单仅在**首次安装**时注入；之后若手动清空，不会自动重新注入（可点「恢复预置」找回）。

### 配置导入 / 导出
在 **⚙️ 高级设置 / API Key** 区域底部：

- **⤓ 导出配置**：将「白名单 + API Key + 情报源开关 + 统计」打包为 `fishing-go-out-config-<日期>.json` 下载，用于备份或换机迁移
- **⤒ 导入配置**：选择导出的 JSON 文件，覆盖当前同名配置后弹窗自动刷新

> 导出的 JSON 含**明文 API Key**，请妥善保管，不要随意外传。
...
- **URLhaus Auth-Key**：在 [auth.abuse.ch](https://auth.abuse.ch/) 免费申请，自 2024 年起 URLhaus 查询**必须**携带 Auth-Key，否则返回 `401`；填入后 URLhaus 才参与拦截
...
- 收集全球恶意软件分发 URL
- **需免费 Auth-Key**（[auth.abuse.ch](https://auth.abuse.ch/) 申请），通过 `Auth-Key` 请求头携带；未配置时自动跳过，不报错
...
- 定位为**确认层**：在情报源列表中靠后，仅当其他源均未命中时才触发，以节省免费配额
...
- 右键手动检查不读缓存，始终实时查询
- **两类白名单请区分**：
  - **持久化白名单（免检测）**：在弹窗「我的白名单」中添加，保存于 `chrome.storage.local`，**重启浏览器不丢**，命中后跳过检测直接放行
  - **会话放行（仅本次放行）**：在阻断页点「我了解风险，继续访问」后加入，仅当前会话有效，重启浏览器后清空，且**不会**自动进入持久化白名单（避免误把危险站永久放行）
- URLhaus 需配置免费 Auth-Key 才参与查询；未配置时自动跳过，不影响其它情报源
...
- **Pulsedive** 免费账户按秒/天/月限速，高频浏览可能触发 429 限制，此时自动跳过 Pulsedive 查询，不影响其他源的拦截能力；可在响应头 `X-Requests-Remaining-Month` 查看当月剩余额度
