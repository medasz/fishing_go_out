...
- **添加**：手动输入域名（自动去协议/路径/端口，转小写）后回车，或点 **+ 添加当前页面域名** 一键加入正在浏览的站点
- **移除**：每条右侧 `✕` 删除单个域名
- **恢复预置**：误删后一键补齐 41 个预置站点（不会删掉你的自定义项）
- **清空白名单**：清空全部自定义与预置项

匹配规则：精确匹配 `github.com`，并自动覆盖子域名 `gist.github.com`、`www.github.com`；不会误命中 `evilgithub.com` 或 `github.com.evil.com`。命中白名单的域名在导航前直接放行、跳过检测。
